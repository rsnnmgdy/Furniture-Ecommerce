...
exports.getAllProducts = asyncHandler(async (req, res) => {
  ...
  const products = await Product.find(query).sort(sort).limit(Number(limit)).skip(skip).select('-__v');
...
exports.createProduct = asyncHandler(async (req, res) => {
  ...
  if (req.files && req.files.length > 0) {
    ...
    req.files.forEach((f, i) => {
      console.log(`  [${i}] ${f.originalname} - Buffer: ${f.buffer ? 'yes (' + f.size + ' bytes)' : 'no'}, Path: ${f.path || 'none'}`);
    });
  }
  ...
  if (req.files && req.files.length > 0) {
    ...
    productData.images = uploaded.map((img, index) => ({ url: img.url, publicId: img.publicId, isPrimary: index === 0 }));
  }
...
exports.updateProduct = asyncHandler(async (req, res) => {
  ...
  if (product.images.length > 0 && !product.images.some(img => img.isPrimary)) product.images[0].isPrimary = true;
  ...
  if (req.body.color) product.color = Array.isArray(req.body.color) ? req.body.color : JSON.parse(req.body.color);
